...
- Code reviews are not about the reviewer or the code creator.
Code reviews are about making the code better.
If at the end of the review, your code is better, it is a good code review.
You have also learned from that review, and should incorporate those suggestions
in not just this code, but in the files that you create in the future.
If at the end of the review, your code is the same, you have written good code.

- A Container contains packaged OS and code.
If you have tested your code inside a container and verified that it works,
you can deploy the container.
This ensures that what the developer sees is what the test engineer sees.
If you deploy that container to production, it it also what the customer experiences.
Keeping all three parties (developer, tester, customer) on the same platform
is the benefit of the container.
When bugs come in (as they always do), you know that it is not the
platform differences that are the problem.
